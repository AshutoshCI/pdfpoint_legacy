...
$sql_1 = "SELECT * FROM pdfpoint_supersubcategory WHERE (supercategory_title LIKE '%$name%' OR supercategory_keywords LIKE '%$name%') AND supercategory_privileges = 'Show'";
	...
	<div class="content-full-width">
        <div class="search-wrapper">
          <form action="<?=SITE_SSL_PATH?>search_data" method="post">
            <div class="column dt-sc-three-fourth first">
              <input class="search-input" type = "text" required id = "name" placeholder="Find topic wise Study Material, Ebooks, Subject Notes, Projects,Question paper,Lecture Videos and College event Videos, Research Paper." name="name" >           
            </div>
      ...
      <div class="search_content">
        <table>
		  ...
		  ?>
          <tr>
            <td><img src='<?=SITE_SSL_PATH?>admin/supercategory_long_image/<?=$query3['supercategory_long_image']?>' width='110px' height='150px' title='<?=$query3['supercategory_title']?>' alt='<?=$query3['supercategory_title']?>' /></td>
            <td class="search_tbl_txt">
            <p><strong>Book Name :: <?php echo $query3['supercategory_title']; ?></strong></p>
 <?  if($query3['supercategory_short_description'] != '' && strtolower($query3['supercategory_short_description']) != 'n/a' ) { ?>
		   <p class="des"><?=$query3['supercategory_short_description']?> </p></td>
 <?}?><a href="<?=SITE_SSL_PATH?>article/<?=$query3['category_id']?>/<?=$query3['supercategory_id']?>">Read More...</a>
		  </tr>
          <?} }else {?>
          <tr>
